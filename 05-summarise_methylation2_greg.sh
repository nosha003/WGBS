...
########## Run #################
        ...
        python /home/springer/nosha003/software/bsmap-2.74/methratio.py \
        -o BSMAPratio/${ID}_methratio2_greg.txt \
        -d ${genome_reference} \
        -u \
        -z \
        -r bsmapped/${ID}_sorted_greg.bam
        ...
        awk -F$"\\t" "$awk_make_bed" \
        "BSMAPratio/${ID}_methratio2.txt" > "BSMAPratio/${ID}_BSMAP_out2.txt"
        ...
        awk_make_bedGraph_context='BEGIN {OFS = FS} (NR>1){
          print $1, $2, $3, $4 > "BSMAPratio/"ID"_BSMAP_out_"$5"_greg.bedGraph"
        }
        '

        #pipe bedGraph to split by context (use dash to read from sdtin)
        awk -F$"\\t" "$awk_make_bedGraph" \
        "BSMAPratio/${ID}_BSMAP_out2.txt" | \
        awk -F$"\\t" -v ID=$ID "$awk_make_bedGraph_context" -

        #Make bigWigs
        /home/springer/nosha003/software/kentUtils/bin/bedGraphToBigWig "BSMAPratio/${ID}_BSMAP_out_CG.bedGraph" /home/springer/nosha003/pete/chr_size.txt \
        "BSMAPratio/${ID}_BSMAP_out_CG.bigWig"
        /home/springer/nosha003/software/kentUtils/bin/bedGraphToBigWig "BSMAPratio/${ID}_BSMAP_out_CHG.bedGraph" /home/springer/nosha003/pete/chr_size.txt \
        "BSMAPratio/${ID}_BSMAP_out_CHG.bigWig"
        /home/springer/nosha003/software/kentUtils/bin/bedGraphToBigWig "BSMAPratio/${ID}_BSMAP_out_CHH.bedGraph" /home/springer/nosha003/pete/chr_size.txt \
	...
	# check if there is PT CHH data
        ...
        BSMAPratio/${ID}_BSMAP_out_greg.txt |wc -l)
        echo $PT_data
	...
	then
	  ...
	  awk -F$"\\t" \
          'BEGIN {OFS = FS} {if($1=="Pt" && $5=="CHH" && $8 != 0) print}' \
          BSMAPratio/${ID}_BSMAP_out2.txt | \
          awk '{sum1 += $9; sum2 +=$8} END {print sum1, sum2 , 100-((sum2/sum1)*100)}' > ConversionRate/${ID}_conversion_rate2.txt
          # conversion rate pete - using eff_CT
          awk -F$"\\t" \
          'BEGIN {OFS = FS} {if($1=="Pt" && $5=="CHH" && $8 != 0) print}' \
          BSMAPratio/${ID}_BSMAP_out2.txt | \
          awk '{sum1 += $7; sum2 +=$8} END {print sum1, sum2 , 100-((sum2/sum1)*100)}' > ConversionRate/${ID}_conversion_rate2_eff_C.txt
	else	
